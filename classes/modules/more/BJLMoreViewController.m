...
- (void)makeSubviews {
    self.backgroundView = ({
        ...
        imageView.image = [[UIImage bjl_imageNamed:@"bjl_menubg"] bjl_resizableImage];
...
- (UIButton *)makeButtonWithTitle:(NSString *)title
    ...
    [button setImage:[UIImage bjl_imageNamed:imageName] forState:UIControlStateNormal];
    if (selectedImageName) {
        UIImage *selectedImage = [UIImage bjl_imageNamed:selectedImageName];
    ...
    for (UIButton *button in buttons) {
        [button bjl_makeConstraints:^(BJLConstraintMaker *make) {
            if (!last) {
                ...
                make.left.equalTo(last.bjl_right).with.offset(BJLViewSpaceM);
            }
