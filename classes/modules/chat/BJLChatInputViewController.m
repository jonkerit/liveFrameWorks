...
- (void)makeSubviews {
    ...
    self.emoticonButton = ({
        UIButton *button = [UIButton new];
        [button setImage:[UIImage bjl_imageNamed:@"bjl_ic_emotion"] forState:UIControlStateNormal];
        [button setImage:[UIImage bjl_imageNamed:@"bjl_ic_emotion"] forState:UIControlStateHighlighted];
        [button setImage:[UIImage bjl_imageNamed:@"bjl_ic_keybord"] forState:UIControlStateSelected];
        [button setImage:[UIImage bjl_imageNamed:@"bjl_ic_keybord"] forState:UIControlStateSelected | UIControlStateHighlighted];
        button;
    });
    ...
    if (self.room.loginUser.isTeacherOrAssistant
        ...
        self.imageButton = ({
            UIButton *button = [UIButton new];
            [button setImage:[UIImage bjl_imageNamed:@"bjl_ic_img"] forState:UIControlStateNormal];
            button;
        });
...
- (void)makeConstraints {
    ...
    [self.emoticonButton bjl_makeConstraints:^(BJLConstraintMaker *make) {
        // 可发图片
        if (self.imageButton) {
            make.left.equalTo(self.imageButton.bjl_right).with.offset(BJLViewSpaceS);
        }
        else {
            make.left.equalTo(self.contentView.bjl_safeAreaLayoutGuide ?: self.contentView).with.offset(BJLViewSpaceS);
        }
        ...
        if (self.privateChatButton) {
            make.right.equalTo(self.privateChatButton.bjl_left).offset(- BJLViewSpaceS);
        }
        else {
            make.right.equalTo(self.textView.bjl_left).offset(- BJLViewSpaceS);
        }
        make.bottom.equalTo(self.textView).with.offset((iconButtonSize - textViewMinHeight) / 2);
        make.width.height.equalTo(@(iconButtonSize));
    }];
    ...
    /*
    [self.sendButton bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.left.equalTo(self.textView.bjl_right).with.offset(BJLViewSpaceM);
        make.right.equalTo(self.contentView.bjl_safeAreaLayoutGuide ?: self.contentView).with.offset(- BJLViewSpaceM);
