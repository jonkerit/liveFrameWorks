...
- (void)makeObserving {
    ...
    if (self.userStates & BJLUserStateMask_speaking) {
        ...
        [self bjl_observe:BJLMakeMethod(self.room.playingVM, playingUserDidUpdate:old:)
                 observer:(BJLMethodFilter)^BOOL(BJLUser * _Nullable now, BJLUser * _Nullable old) {
                     bjl_strongify(self);
                     [self.tableView reloadData];
                     return YES;
                 }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == BJLUserState_request) {
        user = [self.room.speakingRequestVM.speakingRequestUsers bjl_objectAtIndex:indexPath.row];
    }
    else if (indexPath.section == BJLUserState_speaking) {
        user = mediaUser = [self.room.playingVM.playingUsers bjl_objectAtIndex:indexPath.row];
    }
    else { // if (indexPath.section == BJLUserState_online)
        // 在线列表取不到音视频状态
        user = /* mediaUser = */[self.room.onlineUsersVM.onlineUsers bjl_objectAtIndex:indexPath.row];
    }
...
- (void)makeActionsForCell:(BJLUserCell *)cell userState:(BJLUserState)userState {
    ...
    if (userState == BJLUserState_request) {
        ...
        cell.toggleVideoPlayingRequestCallback = cell.toggleVideoPlayingRequestCallback ?: ^(BJLUserCell * _Nullable cell) {
            bjl_strongify(self);
            NSIndexPath *indexPath = [self.tableView indexPathForCell:cell];
            BJLMediaUser *user = [self.room.playingVM.playingUsers bjl_objectAtIndex:indexPath.row];
            BOOL playVideo = user.videoOn && ![self.room.playingVM.videoPlayingUsers containsObject:user];
            [self.room.playingVM updatePlayingUserWithID:user.ID videoOn:playVideo mediaSource:user.mediaSource];
        ...
        cell.stopSpeakingRequestCallback = cell.stopSpeakingRequestCallback ?: ^(BJLUserCell * _Nullable cell) {
            bjl_strongify(self);
            NSIndexPath *indexPath = [self.tableView indexPathForCell:cell];
            BJLUser *user = [self.room.playingVM.playingUsers bjl_objectAtIndex:indexPath.row];
