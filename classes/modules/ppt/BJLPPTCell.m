...
- (void)makeConstraints {
    ...
    [self.nameLabel bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.left.equalTo(self.iconView.bjl_right).with.offset(BJLViewSpaceM);
        make.centerY.equalTo(self.contentView);
    }];
    
    [self.stateLabel bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.horizontal.hugging.compressionResistance.required();
        make.left.greaterThanOrEqualTo(self.nameLabel.bjl_right).with.offset(BJLViewSpaceM);
...
- (UIImage *)iconWithImageFile:(ICLImageFile *)imageFile {
    return [UIImage bjl_imageNamed:@"bjl_ic_file_jpg"]; // thumbnail
}
...
- (UIImage *)iconWithDocument:(BJLDocument *)document {
    ...
    return [UIImage bjl_imageNamed:imageName ?: @"bjl_ic_file_jpg"]; // thumbnail
}
