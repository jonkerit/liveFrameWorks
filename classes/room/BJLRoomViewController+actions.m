...
- (void)makeActionsOnViewDidLoad {
    ...
    [self.controlsViewController setCountDownTimerCallback:^(UIButton * _Nullable button) {
        bjl_strongify(self);
        ...
        self.countDownViewController = [[BJLCountDownViewController alloc] initWithRoom:self.room totalTime:0 currentCountDownTime:0 isDecrease:YES];
        ...
        [self.countDownViewController setPublishCountDownTimerCallback:^BOOL(NSInteger totalTime, NSInteger currentCountDownTime, BOOL isDecrease) {
            bjl_strongify(self);
        ...
        [self.countDownViewController setPauseCountDownTimerCallback:^BOOL{
            bjl_strongify(self);
    ...
    [self.controlsViewController setUsersCallback:^(id _Nullable sender) {
        ...
        if (self.room.roomInfo.newRoomGroupType != BJLRoomNewGroupType_normal) {
            [self.overlayViewController showWithContentViewController:self.onlineGroupUsersViewController];
        }
        else {
    ...
    [self.controlsViewController setMoreCallback:^(UIButton * _Nullable button) {
        ...
        [self.moreViewController.view bjl_remakeConstraints:^(BJLConstraintMaker *make) {
            make.edges.equalTo(self.view);
        }];
        [self.moreViewController updateArrowWithRight:button.bjl_left bottom:button.bjl_top];
    ...
    [self.controlsViewController setQuestionCallback:^(UIButton * _Nullable button) {
        bjl_strongify(self);
        self.controlsViewController.questionRedDot.hidden = YES;
        self.overlayViewController.prefersStatusBarHidden = NO;
        [self.overlayViewController showWithContentViewController:self.questionViewController
    ...
    [self.chatViewController setShowImageViewCallback:^(UIImageView *imageView) {
        ...
        [self.imageViewController.view bjl_makeConstraints:^(BJLConstraintMaker *make) {
            make.edges.equalTo(self.view);
        }];
    ...
    [self.recordingStateView bjl_addHandler:^(__kindof UIControl * _Nullable sender) {
        ...
        UIAlertController *alert = [UIAlertController
                                    bjl_lightAlertControllerWithTitle:@"正在录课中"
                                    message:nil
...
- (CGPoint)bjl_panGestureBegan:(__kindof UIPanGestureRecognizer *)gesture view:(UIView *)view {
    ...
    [view bjl_updateConstraints:^(BJLConstraintMaker *make) {
