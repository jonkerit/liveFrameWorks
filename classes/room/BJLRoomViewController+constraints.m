...
- (void)updateConstraintsForHorizontal:(BOOL)isHorizontal {
    ...
    [@[// !!!: NO self.view
     ...
     makeObjectsPerformSelector:@selector(bjl_remakeConstraints:)
     withObject:^(BJLConstraintMaker *make) {}];
    ...
    [self.recordingStateView bjl_remakeConstraints:^(BJLConstraintMaker *make) {
    ...
    [self.timerView bjl_remakeConstraints:^(BJLConstraintMaker *make) {
        make.edges.equalTo(self.view);
    }];

    [self.overlayViewController.view bjl_remakeConstraints:^(BJLConstraintMaker *make) {
        make.edges.equalTo(self.view);
    }];
    
    [self.loadingViewController.view bjl_remakeConstraints:^(BJLConstraintMaker *make) {
        make.edges.equalTo(self.view);
    }];
...
- (void)updateRecordingStateViewForHorizontal:(BOOL)isHorizontal {
    self.recordingStateView.hidden = (!self.room.loginUser.isTeacherOrAssistant
...
- (void)updatePreviewsAndContentConstraintsForHorizontal:(BOOL)isHorizontal {
    ...
    [self.previewsViewController.view bjl_remakeConstraints:^(BJLConstraintMaker *make) {
        make.left.right.equalTo(self.view);
        make.top.equalTo(isHorizontal ? self.view : self.contentView.bjl_bottom);
    ...
    [self.contentView bjl_remakeConstraints:^(BJLConstraintMaker *make) {
        if (isHorizontal) {
            ...
            make.top.equalTo(showPreviewsInline ? self.view : self.previewsViewController.view.bjl_bottom);
        }
        else {
            make.left.right.top.equalTo(self.view).insets(insets);
            // !!!: .priorityHigh() - fix warnings during ver to hor
            make.height.equalTo(self.contentView.bjl_width).multipliedBy(3.0 / 4).priorityHigh();
        }
    }];
    [self.contentView updateViewsForHorizontal:isHorizontal];
...
- (void)updateControlsConstraintsForHorizontal:(BOOL)isHorizontal {
    ...
    [self.controlsViewController.view bjl_remakeConstraints:^(BJLConstraintMaker *make) {
        // !!!: .priorityHigh() - fix warnings during ver to hor
        make.top.equalTo(self.previewsViewController.view.bjl_bottom).priorityHigh();
        if (isHorizontal) {
            BJLConstraintRelationMaker *xAxisAnchor = (self.controlsHidden ? make.right : make.left);
            xAxisAnchor.equalTo(self.view.bjl_left).with.offset(self.controlsHidden ? 0.0 : insets.left); // maybe update
...
- (void)updateChatConstraintsForHorizontal:(BOOL)isHorizontal {
    [self.chatViewController.view bjl_remakeConstraints:^(BJLConstraintMaker *make) {
        BJLConstraintRelationMaker *horAlignment;
        ...
        horAlignment.equalTo(self.controlsViewController.view.bjl_left).with.offset(leftOffset); // maybe update
        make.right.lessThanOrEqualTo(self.controlsViewController.rightLayoutGuide).with.offset(- BJLViewSpaceM).priorityHigh();
        make.top.equalTo(self.previewsViewController.view.bjl_bottom);
