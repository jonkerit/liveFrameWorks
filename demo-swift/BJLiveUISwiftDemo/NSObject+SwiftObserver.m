...
- (id <BJLObservation>)bjl_observeOnlineUsersDidOverwriteForTarget:(BJLOnlineUsersVM *)onlineUsersVM
                                                            filter:(nullable BOOL (^)(NSArray<BJLUser *> *onlineUsers))filter
                                                          observer:(BOOL (^)(NSArray<BJLUser *> *onlineUsers))observer {
...
- (id <BJLObservation>)bjl_observePlayingUsersDidOverwriteForTarget:(BJLPlayingVM *)playingVM
                                                             filter:(nullable BOOL (^)(NSArray<BJLUser *> *playingUsers))filter
                                                           observer:(BOOL (^)(NSArray<BJLUser *> *playingUsers))observer {
...
- (id <BJLObservation>)bjl_observePlayingUserDidUpdateForTarget:(BJLPlayingVM *)playingVM
                                                         filter:(nullable BOOL (^)(BJLUser *now, BJLUser *old))filter
                                                       observer:(BOOL (^)(BJLUser *now, BJLUser *old))observer {
...
#pragma mark - BJLDocumentVM observe

- (id <BJLObservation>)bjl_observeAllDocumentsDidOverwriteForTarget:(BJLDocumentVM *)documentVM
                                                             filter:(nullable BOOL (^)(NSArray<BJLDocument *> *allDocuments))filter
                                                           observer:(BOOL (^)(NSArray<BJLDocument *> *allDocuments))observer {
    return [self bjl_observe:[BJLMethodMeta instanceWithTarget:documentVM name:@"allDocumentsDidOverwrite:"]
                      filter:filter
                    observer:observer];
}
    
- (id <BJLObservation>)bjl_observeDidAddDocumentForTarget:(BJLDocumentVM *)documentVM
                                                   filter:(nullable BOOL (^)(BJLDocument *documents))filter
                                                 observer:(BOOL (^)(BJLDocument *document))observer {
    return [self bjl_observe:[BJLMethodMeta instanceWithTarget:documentVM name:@"didAddDocument:"]
                      filter:filter
                    observer:observer];
}
    
- (id <BJLObservation>)bjl_observeDidDeleteDocumentForTarget:(BJLDocumentVM *)documentVM
                                                      filter:(nullable BOOL (^)(BJLDocument *documents))filter
                                                    observer:(BOOL (^)(BJLDocument *document))observer {
    return [self bjl_observe:[BJLMethodMeta instanceWithTarget:documentVM name:@"didDeleteDocument:"]
                      filter:filter
                    observer:observer];
}
