...
@property (nonatomic,strong) UIButton *closeNewButton, *serverRecordingButton, *shareButton, *settingButton,*closeButton;
@property (nonatomic, strong) UIImageView *serverRecordingImg;
...
- (void)makeSubviews {
    ...
    [self.view addSubview:self.serverRecordingImg];
    ...
    [self.serverRecordingImg bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.right.equalTo(self.serverRecordingButton.bjl_left).offset(-4);
        make.centerY.equalTo(self.serverRecordingButton);
...
- (UIImageView *)serverRecordingImg{
    if (!_serverRecordingImg) {
        _serverRecordingImg = [[UIImageView alloc] init];
        _serverRecordingImg.backgroundColor = [UIColor bjl_colorWithHexString:@"#00D52C" alpha:1.0];
        _serverRecordingImg.layer.cornerRadius = 4;
        _serverRecordingImg.layer.masksToBounds = YES;
    }
    return _serverRecordingImg;
}
@end
